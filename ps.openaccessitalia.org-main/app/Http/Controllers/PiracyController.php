...
class PiracyController extends Controller
{
            ...
            if (count($check_env) == 0) {
                ...
                if ($all_tickets !== false) {
                    ...
                    foreach ($all_tickets as $ticket) {
                        ...
                        if (! $db_ticket) {
                            ActionLogController::log(0, 'piracy_cron', "ticket $ticket->ticket_id not stored");
                            // store ticket data
                            $new_db_ticket = new Tickets;
                            ...
                            if ($new_db_ticket->save()) {
                                ActionLogController::log(0, 'piracy_cron', "ticket $ticket->ticket_id stored");
                                // check if the ticket is updatable (not older than 48 hours)
                                if (self::is_updatable($ticket->metadata->created_at)) {
                                    // if is updatable
                                    ActionLogController::log(0, 'piracy_cron', "ticket $ticket->ticket_id is updatable");
                                    // check if each item is already in a blocked list
                                    foreach ($ticket->fqdn as $fqdn) {
                                        if (! FQDNs::find($fqdn)) {
                                            ActionLogController::log(0, 'piracy_cron', "ticket $ticket->ticket_id item fqdn $fqdn not stored");
                                            if (self::set_item_processed($fqdn)) {
                                                ActionLogController::log(0, 'piracy_cron', "ticket $ticket->ticket_id item fqdn $fqdn set processed");
                                                $new_db_fqdn = new FQDNs;
                                                $new_db_fqdn->fqdn = $fqdn;
                                                $new_db_fqdn->original_ticket_id = $ticket->ticket_id;
                                                $new_db_fqdn->save();
                                                $new_log = new TicketItemsLog;
                                                ...
                                                ActionLogController::log(0, 'piracy_cron', "ticket $ticket->ticket_id item fqdn $fqdn failed to set processed", true);
                                            }
                                        } else {
                                            ActionLogController::log(0, 'piracy_cron', "ticket $ticket->ticket_id item fqdn $fqdn already stored");
                                            if (self::set_item_unprocessed($fqdn, 'ALREADY_BLOCKED')) {
                                                ActionLogController::log(0, 'piracy_cron', "ticket $ticket->ticket_id item fqdn $fqdn set unprocessed (ALREADY_BLOCKED)");
                                                $new_log = new TicketItemsLog;
                                                $new_log->ticket_id = $ticket->ticket_id;
                                                $new_log->item_type = 'fqdn';
                                                $new_log->item = $fqdn;
                                                ...
                                                ActionLogController::log(0, 'piracy_cron', "ticket $ticket->ticket_id item fqdn $fqdn failed to set unprocessed (ALREADY_BLOCKED)", true);
                                            }
                                        }
                                    }
                                    foreach ($ticket->ipv4 as $ipv4) {
                                        if (! IPv4s::find($ipv4)) {
                                            ActionLogController::log(0, 'piracy_cron', "ticket $ticket->ticket_id item ipv4 $ipv4 not stored");
                                            if (self::set_item_processed($ipv4)) {
                                                ActionLogController::log(0, 'piracy_cron', "ticket $ticket->ticket_id item ipv4 $ipv4 set processed");
                                                $new_db_ipv4 = new IPv4s;
                                                ...
                                                ActionLogController::log(0, 'piracy_cron', "ticket $ticket->ticket_id item ipv4 $ipv4 failed to set processed", true);
                                            }
                                        } else {
                                            ActionLogController::log(0, 'piracy_cron', "ticket $ticket->ticket_id item ipv4 $ipv4 already stored");
                                            if (self::set_item_unprocessed($ipv4, 'ALREADY_BLOCKED')) {
                                                ActionLogController::log(0, 'piracy_cron', "ticket $ticket->ticket_id item ipv4 $ipv4 set unprocessed (ALREADY_BLOCKED)");
                                                $new_log = new TicketItemsLog;
                                                $new_log->ticket_id = $ticket->ticket_id;
                                                $new_log->item_type = 'ipv4';
                                                $new_log->item = $ipv4;
                                                ...
                                                ActionLogController::log(0, 'piracy_cron', "ticket $ticket->ticket_id item ipv4 $ipv4 failed to set unprocessed (ALREADY_BLOCKED)", true);
                                            }
                                        }
                                    }
                                    foreach ($ticket->ipv6 as $ipv6) {
                                        if (! IPv6s::find($ipv6)) {
                                            ActionLogController::log(0, 'piracy_cron', "ticket $ticket->ticket_id item ipv6 $ipv6 not stored");
                                            if (self::set_item_processed($ipv6)) {
                                                ActionLogController::log(0, 'piracy_cron', "ticket $ticket->ticket_id item ipv6 $ipv6 set processed");
                                                $new_db_ipv6 = new IPv6s;
                                                ...
                                                ActionLogController::log(0, 'piracy_cron', "ticket $ticket->ticket_id item ipv6 $ipv6 failed to set processed", true);
                                            }
                                        } else {
                                            ActionLogController::log(0, 'piracy_cron', "ticket $ticket->ticket_id item ipv6 $ipv6 already stored");
                                            if (self::set_item_unprocessed($ipv6, 'ALREADY_BLOCKED')) {
                                                ActionLogController::log(0, 'piracy_cron', "ticket $ticket->ticket_id item ipv6 $ipv6 set unprocessed (ALREADY_BLOCKED)");
                                                $new_log = new TicketItemsLog;
                                                $new_log->ticket_id = $ticket->ticket_id;
                                                $new_log->item_type = 'ipv6';
                                                $new_log->item = $ipv6;
                                                ...
                                                ActionLogController::log(0, 'piracy_cron', "ticket $ticket->ticket_id item ipv6 $ipv6 failed to set unprocessed (ALREADY_BLOCKED)", true);
                                            }
                                        }
                                    }
                                } else {
                                    // if is not updatable
                                    ActionLogController::log(0, 'piracy_cron', "ticket $ticket->ticket_id is not updatable");
                                    // check if each item is already in a blocked list
                                    foreach ($ticket->fqdn as $fqdn) {
                                        if (! FQDNs::find($fqdn)) {
                                            ActionLogController::log(0, 'piracy_cron', "ticket $ticket->ticket_id item fqdn $fqdn not stored");
                                            ...
                                            $new_db_fqdn->original_ticket_id = $ticket->ticket_id;
                                            if ($new_db_fqdn->save()) {
                                                ActionLogController::log(0, 'piracy_cron', "ticket $ticket->ticket_id item fqdn $fqdn stored");
                                            } else {
                                                ActionLogController::log(0, 'piracy_cron', "ticket $ticket->ticket_id item fqdn $fqdn failed to be stored", true);
                                            }
                                        } else {
                                            ActionLogController::log(0, 'piracy_cron', "ticket $ticket->ticket_id item fqdn $fqdn already stored");
                                        }
                                    ...
                                    foreach ($ticket->ipv4 as $ipv4) {
                                        if (! IPv4s::find($ipv4)) {
                                            ...
                                            $new_db_ipv4->original_ticket_id = $ticket->ticket_id;
                                            if ($new_db_ipv4->save()) {
                                                ActionLogController::log(0, 'piracy_cron', "ticket $ticket->ticket_id item ipv4 $ipv4 stored");
                                            } else {
                                                ActionLogController::log(0, 'piracy_cron', "ticket $ticket->ticket_id item ipv4 $ipv4 failed to be stored", true);
                                            }
                                        } else {
                                            ActionLogController::log(0, 'piracy_cron', "ticket $ticket->ticket_id item ipv4 $ipv4 already stored");
                                        }
                                    ...
                                    foreach ($ticket->ipv6 as $ipv6) {
                                        if (! IPv6s::find($ipv6)) {
                                            ...
                                            $new_db_ipv6->original_ticket_id = $ticket->ticket_id;
                                            if ($new_db_ipv6->save()) {
                                                ActionLogController::log(0, 'piracy_cron', "ticket $ticket->ticket_id item ipv6 $ipv6 stored");
                                            } else {
                                                ActionLogController::log(0, 'piracy_cron', "ticket $ticket->ticket_id item ipv6 $ipv6 failed to be stored", true);
                                            }
                                        } else {
                                            ActionLogController::log(0, 'piracy_cron', "ticket $ticket->ticket_id item ipv6 $ipv6 already stored");
                                        }
                                ...
                                ActionLogController::log(0, 'piracy_cron', "ticket $ticket->ticket_id failed to store", true);
                            }
                        } else {
                ...
                ActionLogController::log(0, 'piracy_cron', 'run ended');
            } else {
                ActionLogController::log(0, 'piracy_cron', 'run not started because of: '.implode(', ', $check_env), true);
            }
        }
    }
    ...
    public function delete_from_whitelist(Request $request)
    {
        if ($request->filled(['item', 'genre'])) {
            ...
            ActionLogController::log(Auth::user()->id, Auth::user()->name, "trynig to delete from ps whitelist $item ($genre)");
            if (self::delete_whitelist($genre, $item)) {
                ActionLogController::log(Auth::user()->id, Auth::user()->name, "succeded to delete from ps whitelist $item ($genre)");

                return response('', 200);
            }

            ActionLogController::log(Auth::user()->id, Auth::user()->name, "failed to delete from ps whitelist $item ($genre)");
    ...
    public function add_to_whitelist(Request $request)
    {
        if ($request->filled(['item', 'genre', 'attr'])) {
            ActionLogController::log(Auth::user()->id, Auth::user()->name, 'trynig to add to ps whitelist '.$request->input('item').' ('.$request->input('genre').')');
    ...
    {
        ActionLogController::log(Auth::user()->id, Auth::user()->name, "trying to download ps FQDN $line list");
        switch ($line) {
            case 'offline':
                $fqdns_piracy = FQDNs::select('fqdn')->distinct()->pluck('fqdn')->toArray();
                $content = implode("\n", $fqdns_piracy);
                $headers = [
                    'Content-type' => 'text/plain',
                    'Content-Disposition' => sprintf('attachment; filename="%s"', 'fqdns.txt'),
                ];
                ActionLogController::log(Auth::user()->id, Auth::user()->name, "succeded to download ps FQDN $line list");
            ...
            case 'online':
                ...
                ActionLogController::log(Auth::user()->id, Auth::user()->name, "failed to download ps FQDN $line list (online list not available)");

                return response('', 500);
                break;
            default:
                ActionLogController::log(Auth::user()->id, Auth::user()->name, "failed to download ps FQDN $line list (invalid list type)");
    ...
    {
        ActionLogController::log(Auth::user()->id, Auth::user()->name, "trying to download ps IPv4 $line list");
        switch ($line) {
            case 'offline':
                $ipv4s_piracy = IPv4s::select('ipv4')->distinct()->pluck('ipv4')->toArray();
                $content = implode("\n", $ipv4s_piracy);
                $headers = [
                    'Content-type' => 'text/plain',
                    'Content-Disposition' => sprintf('attachment; filename="%s"', 'ipv4s.txt'),
                ];
                ActionLogController::log(Auth::user()->id, Auth::user()->name, "succeded to download ps IPv4 $line list");
            ...
            case 'online':
                ...
                ActionLogController::log(Auth::user()->id, Auth::user()->name, "failed to download ps IPv4 $line list (online list not available)");

                return response('', 500);
            default:
                ActionLogController::log(Auth::user()->id, Auth::user()->name, "failed to download ps IPv4 $line list (invalid list type)");
    ...
    {
        ActionLogController::log(Auth::user()->id, Auth::user()->name, "trying to download ps IPv6 $line list");
        switch ($line) {
            case 'offline':
                $ipv6s_piracy = IPv6s::select('ipv6')->distinct()->pluck('ipv6')->toArray();
                $content = implode("\n", $ipv6s_piracy);
                $headers = [
                    'Content-type' => 'text/plain',
                    'Content-Disposition' => sprintf('attachment; filename="%s"', 'ipv6s.txt'),
                ];
                ActionLogController::log(Auth::user()->id, Auth::user()->name, "succeded to download ps IPv6 $line list");
            ...
            case 'online':
                ...
                ActionLogController::log(Auth::user()->id, Auth::user()->name, "failed to download ps IPv6 $line list (online list not available)");

                return response('', 500);
                break;
            default:
                ActionLogController::log(Auth::user()->id, Auth::user()->name, "failed to download ps IPv6 $line list (invalid list type)");
                        ...
                        if ($obj->status === 'success') {
                            if (property_exists($obj, 'data')) {
                                if (property_exists($obj->data, 'access_token')) {
                                    ActionLogController::log(0, 'piracy_system', 'succeded to authenticate');
                                    self::api_log('POST', '/api/v1/authentication/login', null, json_encode($body_request), $response->getStatusCode(), $result);

                                    return $obj->data;
                                }

                                ActionLogController::log(0, 'piracy_system', 'failed to authenticate (no access_token property)');
                            } else {
                                ActionLogController::log(0, 'piracy_system', 'failed to authenticate (no data property)');
                            }
                        } else {
                            ActionLogController::log(0, 'piracy_system', 'failed to authenticate (status: '.$obj->status.')');
                        }
                    } else {
                        ActionLogController::log(0, 'piracy_system', 'failed to authenticate (malformed JSON body)');
                    }
                } else {
                    ActionLogController::log(0, 'piracy_system', 'failed to authenticate (not JSON body)');
                }
            } else {
                ActionLogController::log(0, 'piracy_system', 'failed to authenticate (no body in response)');
            }
    ...
    {
        ActionLogController::log(0, 'piracy_system', "trying to refresh token $token");
                ...
                if (self::isJson($result)) {
                    ...
                    if (property_exists($obj, 'status')) {
                        if ($obj->status == 'success') {
                            if (property_exists($obj, 'data')) {
                                if (property_exists($obj->data, 'access_token')) {
                                    ActionLogController::log(0, 'piracy_system', 'succeded to refresh');
                                    self::api_log('POST', '/api/v1/authentication/refresh', null, json_encode($body_request), $response->getStatusCode(), $result);

                                    return $obj->data;
                                } else {
                                    ActionLogController::log(0, 'piracy_system', 'failed to refresh (no access_token property)');
                                }
                            } else {
                                ActionLogController::log(0, 'piracy_system', 'failed to refresh (no data property)');
                            }
                        } else {
                            ActionLogController::log(0, 'piracy_system', 'failed to refresh (status: '.$obj->status.')');
                        }
                    } else {
                        ActionLogController::log(0, 'piracy_system', 'failed to refresh (malformed JSON body)');
                    }
                } else {
                    ActionLogController::log(0, 'piracy_system', 'failed to refresh (not JSON body)');
                }
            } else {
                ActionLogController::log(0, 'piracy_system', 'failed to refresh (no body in response)');
            }
    ...
    {
        ActionLogController::log(0, 'piracy_system', 'trying to logout');
        ...
        if ($access_token) {
            ...
            try {
                ...
                if ($response->getBody()) {
                    ...
                    if (self::isJson($result)) {
                        ...
                        if (property_exists($obj, 'status')) {
                            if ($obj->status == 'success') {
                                if (property_exists($obj, 'data')) {
                                    ActionLogController::log(0, 'piracy_system', 'succeded to logout');
                                    self::api_log('GET', '/api/v1/authentication/logout', $access_token, null, $response->getStatusCode(), $result);

                                    return $obj->data;
                                } else {
                                    ActionLogController::log(0, 'piracy_system', 'failed to logout (no data property)');
                                }
                            } else {
                                ActionLogController::log(0, 'piracy_system', 'failed to logout (status: '.$obj->status.')');
                            }
                        } else {
                            ActionLogController::log(0, 'piracy_system', 'failed to logout (malformed JSON body)');
                        }
                    } else {
                        ActionLogController::log(0, 'piracy_system', 'failed to logout (not JSON body)');
                    }
                } else {
                    ActionLogController::log(0, 'piracy_system', 'failed to logout (no body in response)');
                }
                ...
                if ($e->hasResponse()) {
                    ...
                    ActionLogController::log(0, 'piracy_system', 'failed to logout ('.$e->getResponse().')');
                } else {
                    ActionLogController::log(0, 'piracy_system', 'failed to logout (connection error)');
                }

                return false;
            }
        } else {
            ActionLogController::log(0, 'piracy_system', 'failed to logout (already unauthenticated)');
    ...
    {
        ActionLogController::log(0, 'piracy_system', "trying to get ticket $ticket_id data");
            ...
            try {
                ...
                if ($response->getBody()) {
                    ...
                    if (self::isJson($result)) {
                        ...
                        if (property_exists($obj, 'status')) {
                            if ($obj->status === 'success') {
                                ActionLogController::log(0, 'piracy_system', "succeded to get ticket $ticket_id data");

                                return $obj->data;
                            }

                            ActionLogController::log(0, 'piracy_system', "failed to get ticket $ticket_id data (status: ".$obj->status.')');
                        } else {
                            ActionLogController::log(0, 'piracy_system', "failed to get ticket $ticket_id data (malformed JSON body)");
                        }
                    } else {
                        ActionLogController::log(0, 'piracy_system', "failed to get ticket $ticket_id data (not JSON body)");
                    }
                } else {
                    ActionLogController::log(0, 'piracy_system', "failed to get ticket $ticket_id data (no body in response)");
                }
                ...
                if ($e->hasResponse()) {
                    ...
                    switch ($e->getResponse()->getStatusCode()) {
                        ...
                        case 403:
                            if (! $is_retry) {
                                ActionLogController::log(0, 'piracy_system', 'access token is expired, getting new one');
                                $access_token = self::get_access_token(true);
                                if ($access_token) {
                                    return self::get_ticket($ticket_id, true);
                                }

                                ActionLogController::log(0, 'piracy_system', 'access token is expired, auth failed');
                            } else {
                                ActionLogController::log(0, 'piracy_system', 'access token is expired, not retrying');
                            }
                            break;
                        default:
                            ActionLogController::log(0, 'piracy_system', "failed to get ticket $ticket_id data (".$e->getResponse().')');

                            return false;
                            break;
                    }
                } else {
                    ActionLogController::log(0, 'piracy_system', "failed to get ticket $ticket_id data (connection error)");

                    return false;
                }
            }
        } else {
            ActionLogController::log(0, 'piracy_system', "failed to get ticket $ticket_id data (unauthenticated)");
        }
    ...
    {
        ActionLogController::log(0, 'piracy_system', 'trying to get all tickets');
                ...
                if ($response->getBody()) {
                    ...
                    if (self::isJson($result)) {
                        ...
                        if (property_exists($obj, 'status')) {
                            if ($obj->status === 'success') {
                                if (property_exists($obj, 'data')) {
                                    ActionLogController::log(0, 'piracy_system', 'succeded to get all tickets');

                                    return $obj->data;
                                }

                                ActionLogController::log(0, 'piracy_system', 'failed to get all tickets (no data property)');
                            } else {
                                ActionLogController::log(0, 'piracy_system', 'failed to get all tickets (status: '.$obj->status.')');
                            }
                        } else {
                            ActionLogController::log(0, 'piracy_system', 'failed to get all tickets (malformed JSON body)');
                        }
                    } else {
                        ActionLogController::log(0, 'piracy_system', 'failed to get all tickets (not JSON body)');
                    }
                } else {
                    ActionLogController::log(0, 'piracy_system', 'failed to get all tickets (no body in response)');
                }
                ...
                if ($e->hasResponse()) {
                    ...
                    switch ($e->getResponse()->getStatusCode()) {
                        ...
                        case 403:
                            ...
                            break;
                        default:
                            ActionLogController::log(0, 'piracy_system', 'failed to get all tickets ('.$e->getResponse().')');

                            return false;
                            break;
                    }
                } else {
                    ActionLogController::log(0, 'piracy_system', 'failed to get all tickets (connection error)');

                    return false;
                }
            }
        } else {
            ActionLogController::log(0, 'piracy_system', 'failed to get all tickets (unauthenticated)');
        }
    ...
    {
        ActionLogController::log(0, 'piracy_system', "trying to get fqdns from ticket $ticket_id");
            ...
            try {
                ...
                if ($response->getBody()) {
                    ...
                    if (self::isJson($result)) {
                        ...
                        if (property_exists($obj, 'status')) {
                            if ($obj->status === 'success') {
                                ActionLogController::log(0, 'piracy_system', "succeded to get fqdns from ticket $ticket_id");

                                return $obj->data;
                            }

                            ActionLogController::log(0, 'piracy_system', "failed to get fqdns from ticket $ticket_id (status: ".$obj->status.')');
                        } else {
                            ActionLogController::log(0, 'piracy_system', "failed to get fqdns from ticket $ticket_id (malformed JSON body)");
                        }
                    } else {
                        ActionLogController::log(0, 'piracy_system', "failed to get fqdns from ticket $ticket_id (not JSON body)");
                    }
                } else {
                    ActionLogController::log(0, 'piracy_system', "failed to get fqdns from ticket $ticket_id (no body in response)");
                }
                ...
                if ($e->hasResponse()) {
                    ...
                    switch ($e->getResponse()->getStatusCode()) {
                        ...
                        case 403:
                            ...
                            break;
                        default:
                            ActionLogController::log(0, 'piracy_system', "failed to get fqdns from ticket $ticket_id (".$e->getResponse().')');

                            return false;
                    }
                } else {
                    ActionLogController::log(0, 'piracy_system', "failed to get fqdns from ticket $ticket_id (connection error)");

                    return false;
                }
            }
        } else {
            ActionLogController::log(0, 'piracy_system', "failed to get fqdns from ticket $ticket_id (unauthenticated)");
        }
    ...
    {
        ActionLogController::log(0, 'piracy_system', "trying to get ipv4s from ticket $ticket_id");
            ...
            try {
                ...
                if ($response->getBody()) {
                    ...
                    if (self::isJson($result)) {
                        ...
                        if (property_exists($obj, 'status')) {
                            if ($obj->status === 'success') {
                                ActionLogController::log(0, 'piracy_system', "succeded to get ipv4s from ticket $ticket_id");

                                return $obj->data;
                            }

                            ActionLogController::log(0, 'piracy_system', "failed to get ipv4s from ticket $ticket_id (status: ".$obj->status.')');
                        } else {
                            ActionLogController::log(0, 'piracy_system', "failed to get ipv4s from ticket $ticket_id (malformed JSON body)");
                        }
                    } else {
                        ActionLogController::log(0, 'piracy_system', "failed to get ipv4s from ticket $ticket_id (not JSON body)");
                    }
                } else {
                    ActionLogController::log(0, 'piracy_system', "failed to get ipv4s from ticket $ticket_id (no body in response)");
                }
                ...
                if ($e->hasResponse()) {
                    ...
                    switch ($e->getResponse()->getStatusCode()) {
                        ...
                        case 403:
                            ...
                            break;
                        default:
                            ActionLogController::log(0, 'piracy_system', "failed to get ipv4s from ticket $ticket_id (".$e->getResponse().')');

                            return false;
                    }
                } else {
                    ActionLogController::log(0, 'piracy_system', "failed to get ipv4s from ticket $ticket_id (connection error)");

                    return false;
                }
            }
        } else {
            ActionLogController::log(0, 'piracy_system', "failed to get ipv4s from ticket $ticket_id (unauthenticated)");
        }
    ...
    {
        ActionLogController::log(0, 'piracy_system', "trying to get ipv6s from ticket $ticket_id");
            ...
            try {
                ...
                if ($response->getBody()) {
                    ...
                    if (self::isJson($result)) {
                        ...
                        if (property_exists($obj, 'status')) {
                            if ($obj->status === 'success') {
                                ActionLogController::log(0, 'piracy_system', "succeded to get ipv6s from ticket $ticket_id");

                                return $obj->data;
                            }

                            ActionLogController::log(0, 'piracy_system', "failed to get ipv6s from ticket $ticket_id (status: ".$obj->status.')');
                        } else {
                            ActionLogController::log(0, 'piracy_system', "failed to get ipv6s from ticket $ticket_id (malformed JSON body)");
                        }
                    } else {
                        ActionLogController::log(0, 'piracy_system', "failed to get ipv6s from ticket $ticket_id (not JSON body)");
                    }
                } else {
                    ActionLogController::log(0, 'piracy_system', "failed to get ipv6s from ticket $ticket_id (no body in response)");
                }
                ...
                if ($e->hasResponse()) {
                    ...
                    switch ($e->getResponse()->getStatusCode()) {
                        ...
                        case 403:
                            ...
                            break;
                        default:
                            ActionLogController::log(0, 'piracy_system', "failed to get ipv6s from ticket $ticket_id (".$e->getResponse().')');

                            return false;
                    }
                } else {
                    ActionLogController::log(0, 'piracy_system', "failed to get ipv6s from ticket $ticket_id (connection error)");

                    return false;
                }
            }
        } else {
            ActionLogController::log(0, 'piracy_system', "failed to get ipv6s from ticket $ticket_id (unauthenticated)");
        }
    ...
    {
        ActionLogController::log(0, 'piracy_system', 'trying to get FQDN json list');
                ...
                if ($response->getBody()) {
                    ...
                    if (self::isJson($result)) {
                        ...
                        if (property_exists($obj, 'status')) {
                            if ($obj->status === 'success') {
                                if (property_exists($obj, 'data')) {
                                    ActionLogController::log(0, 'piracy_system', 'succeded to get FQDN json list');

                                    return $obj->data;
                                }

                                ActionLogController::log(0, 'piracy_system', 'failed to get FQDN json list (no data property)');
                            } else {
                                ActionLogController::log(0, 'piracy_system', 'failed to get FQDN json list (status: '.$obj->status.')');
                            }
                        } else {
                            ActionLogController::log(0, 'piracy_system', 'failed to get FQDN json list (malformed JSON body)');
                        }
                    } else {
                        ActionLogController::log(0, 'piracy_system', 'failed to get FQDN json list (not JSON body)');
                    }
                } else {
                    ActionLogController::log(0, 'piracy_system', 'failed to get FQDN json list (no body in response)');
                }
                ...
                if ($e->hasResponse()) {
                    ...
                    switch ($e->getResponse()->getStatusCode()) {
                        ...
                        case 403:
                            ...
                            break;
                        default:
                            ActionLogController::log(0, 'piracy_system', 'failed to get FQDN json list ('.$e->getResponse().')');

                            return false;
                    }
                } else {
                    ActionLogController::log(0, 'piracy_system', 'failed to get FQDN json list (connection error)');

                    return false;
                }
            }
        } else {
            ActionLogController::log(0, 'piracy_system', 'failed to get FQDN json list (unauthenticated)');
        }
    ...
    {
        ActionLogController::log(0, 'piracy_system', 'trying to get IPv4 json list');
                ...
                if ($response->getBody()) {
                    ...
                    if (self::isJson($result)) {
                        ...
                        if (property_exists($obj, 'status')) {
                            if ($obj->status === 'success') {
                                if (property_exists($obj, 'data')) {
                                    ActionLogController::log(0, 'piracy_system', 'succeded to get IPv4 json list');

                                    return $obj->data;
                                }

                                ActionLogController::log(0, 'piracy_system', 'failed to get IPv4 json list (no data property)');
                            } else {
                                ActionLogController::log(0, 'piracy_system', 'failed to get IPv4 json list (status: '.$obj->status.')');
                            }
                        } else {
                            ActionLogController::log(0, 'piracy_system', 'failed to get IPv4 json list (malformed JSON body)');
                        }
                    } else {
                        ActionLogController::log(0, 'piracy_system', 'failed to get IPv4 json list (not JSON body)');
                    }
                } else {
                    ActionLogController::log(0, 'piracy_system', 'failed to get IPv4 json list (no body in response)');
                }
                ...
                if ($e->hasResponse()) {
                    ...
                    switch ($e->getResponse()->getStatusCode()) {
                        ...
                        case 403:
                            ...
                            break;
                        default:
                            ActionLogController::log(0, 'piracy_system', 'failed to get IPv4 json list ('.$e->getResponse().')');

                            return false;
                            break;
                    }
                } else {
                    ActionLogController::log(0, 'piracy_system', 'failed to get IPv4 json list (connection error)');

                    return false;
                }
            }
        } else {
            ActionLogController::log(0, 'piracy_system', 'failed to get IPv4 json list (unauthenticated)');
        }
    ...
    {
        ActionLogController::log(0, 'piracy_system', 'trying to get IPv6 json list');
                ...
                if ($response->getBody()) {
                    ...
                    if (self::isJson($result)) {
                        ...
                        if (property_exists($obj, 'status')) {
                            if ($obj->status === 'success') {
                                if (property_exists($obj, 'data')) {
                                    ActionLogController::log(0, 'piracy_system', 'succeded to get ipv6 json list');

                                    return $obj->data;
                                }

                                ActionLogController::log(0, 'piracy_system', 'failed to get ipv6 json list (no data property)');
                            } else {
                                ActionLogController::log(0, 'piracy_system', 'failed to get ipv6 json list (status: '.$obj->status.')');
                            }
                        } else {
                            ActionLogController::log(0, 'piracy_system', 'failed to get ipv6 json list (malformed JSON body)');
                        }
                    } else {
                        ActionLogController::log(0, 'piracy_system', 'failed to get ipv6 json list (not JSON body)');
                    }
                } else {
                    ActionLogController::log(0, 'piracy_system', 'failed to get ipv6 json list (no body in response)');
                }
                ...
                if ($e->hasResponse()) {
                    ...
                    switch ($e->getResponse()->getStatusCode()) {
                        ...
                        case 403:
                            ...
                            break;
                        default:
                            ActionLogController::log(0, 'piracy_system', 'failed to get ipv6 json list ('.$e->getResponse().')');

                            return false;
                            break;
                    }
                } else {
                    ActionLogController::log(0, 'piracy_system', 'failed to get ipv6 json list (connection error)');

                    return false;
                }
            }
        } else {
            ActionLogController::log(0, 'piracy_system', 'failed to get ipv6 json list (unauthenticated)');
    ...
    {
        ActionLogController::log(0, 'piracy_system', "trying to set $ticket_item processed");
            ...
            try {
                ...
                if ($response->getBody()) {
                    ...
                    if (self::isJson($result)) {
                        ...
                        if (property_exists($obj, 'status')) {
                            if ($obj->status === 'success') {
                                ActionLogController::log(0, 'piracy_system', "succeded to set $ticket_item processed");

                                return true;
                            }

                            ActionLogController::log(0, 'piracy_system', "failed to set $ticket_item processed (status: ".$obj->status.')');
                        } else {
                            ActionLogController::log(0, 'piracy_system', "failed to set $ticket_item processed (malformed JSON body)");
                        }
                    } else {
                        ActionLogController::log(0, 'piracy_system', "failed to set $ticket_item processed (not JSON body)");
                    }
                } else {
                    ActionLogController::log(0, 'piracy_system', "failed to set $ticket_item processed (no body in response)");
                }
                ...
                if ($e->hasResponse()) {
                    ...
                    switch ($e->getResponse()->getStatusCode()) {
                        ...
                        case 403:
                            ...
                            break;
                        default:
                            ActionLogController::log(0, 'piracy_system', "failed to set $ticket_item processed (".$e->getResponse().')');

                            return false;
                            break;
                    }
                } else {
                    ActionLogController::log(0, 'piracy_system', "failed to set $ticket_item processed (connection error)");

                    return false;
                }
            }
        } else {
            ActionLogController::log(0, 'piracy_system', "failed to set $ticket_item processed (unauthenticated)");
        }
    ...
    {
        ActionLogController::log(0, 'piracy_system', "trying to set $ticket_item unprocessed for reason $reason");
            ...
            try {
                ...
                if ($response->getBody()) {
                    ...
                    if (self::isJson($result)) {
                        ...
                        if (property_exists($obj, 'status')) {
                            if ($obj->status === 'success') {
                                ActionLogController::log(0, 'piracy_system', "succeded to set $ticket_item unprocessed for reason $reason");

                                return true;
                            }

                            ActionLogController::log(0, 'piracy_system', "failed to set $ticket_item unprocessed for reason $reason (status: ".$obj->status.')');
                        } else {
                            ActionLogController::log(0, 'piracy_system', "failed to set $ticket_item unprocessed for reason $reason (malformed JSON body)");
                        }
                    } else {
                        ActionLogController::log(0, 'piracy_system', "failed to set $ticket_item unprocessed for reason $reason (not JSON body)");
                    }
                } else {
                    ActionLogController::log(0, 'piracy_system', "failed to set $ticket_item unprocessed for reason $reason (no body in response)");
                }
                ...
                if ($e->hasResponse()) {
                    ...
                    switch ($e->getResponse()->getStatusCode()) {
                        ...
                        case 403:
                            ...
                            break;
                        default:
                            ActionLogController::log(0, 'piracy_system', "failed to set $ticket_item unprocessed for reason $reason (".$e->getResponse().')');

                            return false;
                            break;
                    }
                } else {
                    ActionLogController::log(0, 'piracy_system', "failed to set $ticket_item unprocessed for reason $reason (connection error)");

                    return false;
                }
            }
        } else {
            ActionLogController::log(0, 'piracy_system', "failed to set $ticket_item unprocessed for reason $reason (unauthenticated)");
        }
    ...
    {
        ActionLogController::log(0, 'piracy_system', 'trying to get whitelist');
                ...
                if ($response->getBody()) {
                    ...
                    if (self::isJson($result)) {
                        ...
                        if (property_exists($obj, 'status')) {
                            if ($obj->status == 'success') {
                                if (property_exists($obj, 'data')) {
                                    ActionLogController::log(0, 'piracy_system', 'succeded to get whitelist');

                                    return $obj->data;
                                }

                                ActionLogController::log(0, 'piracy_system', 'failed to get whitelist (no data property)');
                            } else {
                                ActionLogController::log(0, 'piracy_system', 'failed to get whitelist (status: '.$obj->status.')');
                            }
                        } else {
                            ActionLogController::log(0, 'piracy_system', 'failed to get whitelist (malformed JSON body)');
                        }
                    } else {
                        ActionLogController::log(0, 'piracy_system', 'failed to get whitelist (not JSON body)');
                    }
                } else {
                    ActionLogController::log(0, 'piracy_system', 'failed to get whitelist (no body in response)');
                }
                ...
                if ($e->hasResponse()) {
                    ...
                    switch ($e->getResponse()->getStatusCode()) {
                        ...
                        case 403:
                            ...
                            break;
                        default:
                            ActionLogController::log(0, 'piracy_system', 'failed to get whitelist ('.$e->getResponse().')');

                            return false;
                    }
                } else {
                    ActionLogController::log(0, 'piracy_system', 'failed to get whitelist (connection error)');

                    return false;
                }
            }
        } else {
            ActionLogController::log(0, 'piracy_system', 'failed to get whitelist (unauthenticated)');
        }
    ...
    {
        ActionLogController::log(0, 'piracy_system', "trying to delete from whitelist item $item (genre $genre)");
            ...
            try {
                ...
                if ($response->getBody()) {
                    ...
                    if (self::isJson($result)) {
                        ...
                        if (property_exists($obj, 'status')) {
                            if ($obj->status == 'success') {
                                ActionLogController::log(0, 'piracy_system', "succeded to delete from whitelist item $item (genre $genre)");

                                return true;
                            }

                            ActionLogController::log(0, 'piracy_system', "failed to delete from whitelist item $item (genre $genre) (status: ".$obj->status.')');
                        } else {
                            ActionLogController::log(0, 'piracy_system', "failed to delete from whitelist item $item (genre $genre) (malformed JSON body)");
                        }
                    } else {
                        ActionLogController::log(0, 'piracy_system', "failed to delete from whitelist item $item (genre $genre) (not JSON body)");
                    }
                } else {
                    ActionLogController::log(0, 'piracy_system', "failed to delete from whitelist item $item (genre $genre) (no body in response)");
                }
                ...
                if ($e->hasResponse()) {
                    ...
                    switch ($e->getResponse()->getStatusCode()) {
                        ...
                        case 403:
                            ...
                            break;
                        default:
                            ActionLogController::log(0, 'piracy_system', "failed to delete from whitelist item $item (genre $genre) (".$e->getResponse().')');

                            return false;
                            break;
                    }
                } else {
                    ActionLogController::log(0, 'piracy_system', "failed to delete from whitelist item $item (genre $genre) (connection error)");

                    return false;
                }
            }
        } else {
            ActionLogController::log(0, 'piracy_system', "failed to delete from whitelist item $item (genre $genre) (unauthenticated)");
    ...
    {
        ActionLogController::log(0, 'piracy_system', "trying to add to whitelist item $item (genre $genre - $attr_name $attr)");
            ...
            try {
                ...
                if ($response->getBody()) {
                    ...
                    if (self::isJson($result)) {
                        ...
                        if (property_exists($obj, 'status')) {
                            if ($obj->status === 'success') {
                                ActionLogController::log(0, 'piracy_system', "succeded to add to whitelist item $item (genre $genre - $attr_name $attr)");

                                return true;
                            }

                            ActionLogController::log(0, 'piracy_system', "failed to add to whitelist item $item (genre $genre - $attr_name $attr) (status: ".$obj->status.')');
                        } else {
                            ActionLogController::log(0, 'piracy_system', "failed to add to whitelist item $item (genre $genre - $attr_name $attr) (malformed JSON body)");
                        }
                    } else {
                        ActionLogController::log(0, 'piracy_system', "failed to add to whitelist item $item (genre $genre - $attr_name $attr) (not JSON body)");
                    }
                } else {
                    ActionLogController::log(0, 'piracy_system', "failed to add to whitelist item $item (genre $genre - $attr_name $attr) (no body in response)");
                }
                ...
                if ($e->hasResponse()) {
                    ...
                    switch ($e->getResponse()->getStatusCode()) {
                        ...
                        case 403:
                            ...
                            break;
                        default:
                            ActionLogController::log(0, 'piracy_system', "failed to add to whitelist item $item (genre $genre - $attr_name $attr) (".$e->getResponse().')');

                            return false;
                            break;
                    }
                } else {
                    ActionLogController::log(0, 'piracy_system', "failed to add to whitelist item $item (genre $genre - $attr_name $attr) (connection error)");

                    return false;
                }
            }
        } else {
            ActionLogController::log(0, 'piracy_system', "failed to add to whitelist item $item (genre $genre - $attr_name $attr) (unauthenticated)");
    ...
    {
        ActionLogController::log(0, 'piracy_system', 'trying to ping system');
        ...
        try {
            ...
            if ($response->getBody()) {
                ...
                if (self::isJson($result)) {
                    $obj = json_decode($result);
                    if (property_exists($obj, 'status')) {
                        ActionLogController::log(0, 'piracy_system', 'ping result status: '.$obj->status);
                        if ($obj->status === 'success') {
                            return true;
                        }

                    } else {
                        ActionLogController::log(0, 'piracy_system', 'ping result: malformed JSON body');
                    }
                } else {
                    ActionLogController::log(0, 'piracy_system', 'ping result: not JSON body');
                }
            } else {
                ActionLogController::log(0, 'piracy_system', 'ping result: no body in response');
            }
        } catch (RequestException $e) {
            if ($e->hasResponse()) {
                if ($e->getResponse()->getBody()) {
                ...
                ActionLogController::log(0, 'piracy_system', 'ping result: system is offline ('.$e->getResponse().')');
            } else {
                ActionLogController::log(0, 'piracy_system', 'ping result: system is offline (connection error)');
            }
        }
